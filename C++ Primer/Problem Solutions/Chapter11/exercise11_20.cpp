...
private:
    ...
    void increaseWordNum(std::string & word);
    ...
    for(auto beg = line.begin(), end = line.end(); beg!=end; ++beg){
        if(!inWord && (::isalnum(*beg) || *beg == '_') ){
            ...
            if(wordBeg != wordEnd){
                ...
                //++this->wordCount[word]; 代替操作
                increaseWordNum(word);
            }  
        }
    }
}

void WordCounter::increaseWordNum(std::string & word){
    auto ret = this->wordCount.insert(make_pair(word, 1));
    if(!ret.second){
        ++ret.first->second;
    }
}
...
{
    std::string fileName(R"(./exercise11_20.cpp)");
